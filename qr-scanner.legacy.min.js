(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? module.exports = factory(require('jsqr-es6'), require('jsqr-es6/dist/BitMatrix.js'), require('jsqr-es6/dist/decoder/reedsolomon/index.js')) :
    typeof define === 'function' && define.amd ? define(['jsqr-es6', 'jsqr-es6/dist/BitMatrix.js', 'jsqr-es6/dist/decoder/reedsolomon/index.js'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.QrScanner = factory(global.jsQR, global.BitMatrix_js, global.index_js));
})(this, (function (jsQR, BitMatrix_js, index_js) { 'use strict';
    ...
    class QrScanner {
        ...
        constructor(video, onDecode, canvasSizeOrOnDecodeErrorOrOptions, canvasSizeOrCalculateScanRegion, preferredCamera) {
            ...
            if (canvasSizeOrOnDecodeErrorOrOptions &&
            ...
            try {
                ...
                if (qrEngine) {
                    detailedScanResult = await new Promise((resolve, reject) => {
                        ...
                        onMessage = (event) => {
                            ...
                            if (event.data.data !== null) {
                                ...
                                const res = {
                                    ...
                                    matrix: BitMatrix_js.BitMatrix.createFromPacked(data.matrixData, data.matrixWidth, data.matrixWidth),
                                    matrixCorrected: BitMatrix_js.BitMatrix.createFromPacked(data.matrixDataCorrected, data.matrixWidth, data.matrixWidth),
                                ...
                                if (imageOrFileOrBlobOrUrl instanceof HTMLVideoElement) {
                                    canvas.toBlob((blob) => {
                                        if (blob) {
                                            resolve(Object.assign(Object.assign({}, res), { scannedFrame: blob }));
                                        }
                                        else {
                                            resolve(res);
                                        }
                        ...
                        expectedResponseId = QrScanner._postWorkerMessageSync(qrEngine, "decode", imageData, [imageData.data.buffer]);
                    });
                }
                else {
                    // do it without worker
                    if (!index_js.getDecoderInitialized()) {
                        await index_js.initDecoder();
                    }
                    const data = canvasContext.getImageData(0, 0, canvas.width, canvas.height);
                    const res = jsQR__default.default(data.data, data.width, data.height, {
                        inversionAttempts,
                        greyScaleWeights,
                    });
                    if (!res) {
        ...
        static createQrEngine(workerPath) {
            if (workerPath) {
                console.warn("Specifying a worker path is not required and not supported anymore.");
            }
            return new Worker(new URL("./qr-scanner-worker.min.js", (typeof document === 'undefined' && typeof location === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : typeof document === 'undefined' ? location.href : (_documentCurrentScript && _documentCurrentScript.src || new URL('qr-scanner.legacy.min.js', document.baseURI).href))), {
                type: "module",
            });
        }
        _onPlay() {
